...
@entity class ElectronCompilationSources(scope: CompilationScope) extends CompilationSources {

  override def id: ScopeId = scope.id

  @node override protected def hashedSources: HashedSources = {
    val tpe = "Source"
    val rootPath = scope.electronSourceFolders.headOption
      .map(f => f.workspaceSourceRoot.resolveDir(f.workspaceSrcRootToSourceFolderPath).path)
      .getOrElse(Paths.get(""))
    val hashedFiles = scope.electronSourceFolders.apar
      .map { f =>
      ...
      .merge[SourceUnitId]
    val fingerprint = scope.fingerprint(hashedFiles, tpe) ++ scope.electronDependenciesFingerprint
    val fingerprintHash = scope.hasher.hashFingerprint(fingerprint, ArtifactType.ElectronFingerprint)
    HashedSourcesImpl(Seq(tpe -> hashedFiles), Nil, fingerprintHash)
  }
}

object ElectronCompilationSources {
