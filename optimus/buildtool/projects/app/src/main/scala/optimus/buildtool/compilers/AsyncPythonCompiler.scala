...
import optimus.buildtool.compilers.venv.RuffWrapperFactory
import optimus.buildtool.compilers.venv.ThinPyappWrapper._
import optimus.buildtool.compilers.venv.VenvProvider._
...
@entity private[buildtool] class AsyncPythonCompilerImpl(
  ...
  @node private def buildArtifact(scopeId: ScopeId, inputs: NodeFunction0[Inputs]): PythonArtifact = {
    ...
    NodeTry {
      ...
      val sandbox = sandboxFactory.withRootPaths(s"${scopeId.properPath}-python-", sourceFiles)
      Utils.createDirectories(sandbox.buildDir)

      val artifact = Utils.atomicallyWrite(outputTpa) { tempPath =>
        val (buildVenv, buildVenvErrors) =
          ensureVenvExists(pythonConfig.overriddenCommands, pythonConfig.python, pythonEnvironment)

        val buildErrors =
          if (buildVenvErrors.exists(_.isError)) Seq()
          else {
            val linterErrors = lint(buildVenv, sandbox)
            val tpaErrors = createTpa(buildVenv, tempPath, pythonConfig, sandbox, pythonEnvironment, mapping())
            linterErrors ++ tpaErrors
          }

        val buildMessages = buildVenvErrors ++ buildErrors
        val buildFailed = MessagesArtifact.hasErrors(buildMessages)
  ...
  @async private def lint(buildVenv: Path, sandbox: Sandbox): Seq[CompilationMessage] = {
    val (ruff, ruffVenvErrors) = RuffWrapperFactory(buildVenv)
    val linterErrors =
      if (!ruffVenvErrors.exists(_.isError))
        ruff
          .check(
            srcRoot.resolveFile(RelativePath("config/ruff-obt.toml")).path,
            sandbox.rootSourceDir
          )
          .map(_.toCompilationMessage)
      else
        Seq()

    linterErrors ++ ruffVenvErrors
  }
}
