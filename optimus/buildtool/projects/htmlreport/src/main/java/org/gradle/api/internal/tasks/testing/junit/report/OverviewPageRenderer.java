...
class OverviewPageRenderer extends PageRenderer<AllTestResults> {
  ...
  private void renderPackages(SimpleHtmlWriter htmlWriter) throws IOException {
    ...
    for (PackageTestResults testPackage : getResults().getPackages()) {
      ...
      htmlWriter
          .startElement("a")
          .attribute("href", testPackage.getBaseUrl())
          .characters(testPackage.getName())
          .endElement();
      htmlWriter.endElement();
      htmlWriter
          .startElement("td")
          .characters(Integer.toString(testPackage.getTestCount()))
          .endElement();
      htmlWriter
          .startElement("td")
          .characters(Integer.toString(testPackage.getFailureCount()))
          .endElement();
      htmlWriter
          .startElement("td")
          .characters(Integer.toString(testPackage.getIgnoredCount()))
          .endElement();
      htmlWriter.startElement("td").characters(testPackage.getFormattedDuration()).endElement();
      htmlWriter
          .startElement("td")
          .attribute("class", testPackage.getStatusClass())
          .characters(testPackage.getFormattedSuccessRate())
          .endElement();
      htmlWriter.endElement();
    }
    htmlWriter.endElement();
  ...
  private void renderClasses(SimpleHtmlWriter htmlWriter) throws IOException {
    ...
    for (PackageTestResults testPackage : getResults().getPackages()) {
      for (ClassTestResults testClass : testPackage.getClasses()) {
        ...
        htmlWriter
            .startElement("a")
            .attribute("href", asHtmlLinkEncoded(testClass.getBaseUrl()))
