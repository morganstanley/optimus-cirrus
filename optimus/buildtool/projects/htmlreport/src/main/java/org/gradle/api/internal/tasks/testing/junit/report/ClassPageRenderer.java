...
class ClassPageRenderer extends PageRenderer<ClassTestResults> {
  ...
  protected void renderBreadcrumbs(SimpleHtmlWriter htmlWriter) throws IOException {
    htmlWriter
        ...
        .startElement("a")
        .attribute("href", getResults().getUrlTo(getResults().getParent().getParent()))
        .characters("all")
        .endElement()
        .characters(" > ")
        .startElement("a")
        .attribute("href", getResults().getUrlTo(getResults().getPackageResults()))
        .characters(getResults().getPackageResults().getName())
        .endElement()
  ...
  private void renderTests(SimpleHtmlWriter htmlWriter) throws IOException {
    ...
    for (TestResult test : getResults().getTestResults()) {
      htmlWriter
          ...
          .characters(test.getName())
          .endElement()
          .startElement("td")
          .characters(test.getFormattedDuration())
          .endElement()
          .startElement("td")
          .attribute("class", test.getStatusClass())
          .characters(test.getFormattedResultType())
          .endElement()
          .endElement();
    }
  ...
  protected void renderFailures(SimpleHtmlWriter htmlWriter) throws IOException {
    for (TestResult test : getResults().getFailures()) {
      htmlWriter
          ...
          .startElement("a")
          .attribute("name", test.getId().toString())
          .characters("")
      ...
      for (TestFailure failure : test.getFailures()) {
        String message;
        if (GUtil.isTrue(failure.getMessage())
            && !failure.getStackTrace().contains(failure.getMessage())) {
          message =
              failure.getMessage()
          ...
          new ErroringAction<SimpleHtmlWriter>() {
            ...
            protected void doExecute(SimpleHtmlWriter htmlWriter) throws IOException {
              ...
              resultsProvider.writeAllOutput(
                  classId, TestOutputEvent.Destination.StdOut, htmlWriter);
