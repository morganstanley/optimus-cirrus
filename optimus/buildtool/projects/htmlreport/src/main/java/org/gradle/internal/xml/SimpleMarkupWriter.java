...
/**
 * A streaming markup writer. Encodes characters and CDATA. Provides only basic state validation,
 * and some simple indentation.
 *
 * <p>This class also is-a {@link Writer}, and any characters written to this writer will be encoded
 * as appropriate. Note, however, that calling {@link #close()} on this object does not close the
 * backing stream.
 */
    ...
    if (context != Context.StartTag) {
      throw new IllegalStateException(
          "Cannot write attribute ["
              ...
              + ":"
              + value
              + "]. You should write start element first.");
    }
