 ...
 *
 * The only changes from the original are package and interface names and javadoc error fixes. For
 * those changes only:
 *
...
import static appmw.dht.digest.Blake2bUnsafe.Engine.Assert.*;
import static appmw.dht.digest.Blake2bUnsafe.Engine.LittleEndian.*;
...
/** */
public interface Blake2bUnsafe {
  // ---------------------------------------------------------------------
  // Specification
  // ---------------------------------------------------------------------
  public interface Spec {
    ...
    /**
     * max tree leaf length value.Note that this has uint32 semantics and thus 0xFFFFFFFF is used as
     * max value limit.
     */
    int max_tree_leaf_length = 0xFFFFFFFF;

    /**
     * max node offset value. Note that this has uint64 semantics and thus 0xFFFFFFFFFFFFFFFFL is
  ...
  // API
  // ---------------------------------------------------------------------
  // TODO (OPTIMUS-0000): add ByteBuffer variants

  /**
   * A serializable / JSON-izable object usable for pausing a hash-in-process which can then be
   * resumed with the same Parameter the original digest was constructed with, and fed additional
   * bytes to conclude the hash.
   */
  public static final class ResumeHandle implements Serializable {
    ...
    /**
     * Create a reconstituted Blake2b digest from
     *
     * @param param
     * @return
     */
    public Blake2bUnsafe resume(Param param) {
  ...
  public static class Digest extends Engine implements Blake2bUnsafe {
  ...
  public static class Mac extends Engine implements Blake2bUnsafe {
    ...
    /**
     * Blake2b.MAC - using default Blake2b.Spec settings with given java.security.Key, with given
     * digest length
     */
  ...
  // Engine
  // ---------------------------------------------------------------------
  static class Engine implements Blake2bUnsafe {

    // ---------------------------------------------------------------------
    // Blake2b State(+) per reference implementation
    // ---------------------------------------------------------------------
    // REVU: address last_node
    // TODO (OPTIMUS-0000): part of the Tree/incremental
    ...
    /** Basic use constructor pending TODO (OPTIMUS-0000): JCA/JCE compliance */
    Engine() {
      this(new Param());
    }
    ...
    public static void main(String... args) {
      Blake2bUnsafe mac = Blake2bUnsafe.Mac.newInstance("LOVE".getBytes());
    ...
    public static class Debug {
      public static void dumpState(Blake2bUnsafe.Engine e, final String mark) {
  ...
  // REVU: need to review a revert back to non-lazy impl
  // TODO (OPTIMUS-0000): do & bench
  public static class Param implements AlgorithmParameterSpec {
    ...
    /** TODO (OPTIMUS-0000): not sure how to make this secure */
    ...
    // TODO (OPTIMUS-0000): do the same for tree params depth, fanout, inner, node-depth,
    // node-offset
    ...
    public final Param setDigestLength(int len) {
      ...
      assert len <= Spec.max_digest_bytes
          : assertFail("len", len, inclusiveUpperBound, Spec.max_digest_bytes);
