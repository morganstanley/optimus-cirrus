...
public class DHTConnectionFailedException extends DHTException {

  private final ServerConnection server;
  private final List<SerializableKey> keys;

  public DHTConnectionFailedException(
      ServerConnection server, List<? extends Key> keys, Throwable cause) {
    super("Couldn't connect to " + server + " for keys " + formatKeys(keys), cause);
    this.server = server;
