...
public class DHTReadTimeoutException extends DHTException {

  private final ServerConnection server;
  private final Duration timeout;
  private final List<SerializableKey> keys;

  public DHTReadTimeoutException(
      ServerConnection server, Duration timeout, List<? extends Key> keys) {
    super("Read timeout after " + timeout + " for " + server + " for keys " + formatKeys(keys));
    this.server = server;
