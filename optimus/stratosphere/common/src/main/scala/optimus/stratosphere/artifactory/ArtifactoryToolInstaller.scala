...
object ArtifactoryToolInstaller {
  ...
  def presetArtifactoryCredentials(stratoWorkspace: StratoWorkspaceCommon): Unit = {
    ...
    if (
      ...
      try {
        ArtifactoryToolInstaller.setupJfrogUser(stratoWorkspace)
      } catch {
        case NonFatal(e) =>
    ...
    if (
      ...
      try {
        ArtifactoryToolInstaller.setupPypiUser(stratoWorkspace)
      } catch {
        case NonFatal(e) =>
  ...
  @tailrec private def setupGenericUser(
  ...
  def repositoryUrl(jfrogConfigFile: Path): String = {
    ...
    s"""${uri.getScheme}://$user:$password@${url}maven-all-local-only/"""
  }
  ...
  def downloadArtifact(
    ...
    def downloadFromJfrog(): String = {
      ...
      val apiKeyParams = Seq("--password", fetchApiKey())
...
class ArtifactoryToolInstaller(
    stratoWorkspace: StratoWorkspaceCommon,
    name: String,
    requestedVersion: Option[String] = None) {

  import ArtifactoryToolInstaller._
  ...
  def isInstalled: Boolean = install.toolDir.exists()
  ...
  private def extract(): Unit = {
    findInstallers() match {
      ...
      case Seq(installer) if installer.file.name.endsWith(".7z.exe") =>
        ...
        ArtifactoryToolInstaller.noProgress7Zip(installer, noProgressInstaller)
