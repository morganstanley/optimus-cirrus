...
/**
 * ProtoMessageGroup for messages that contains values e.g. put and retrieval response. Values are
 * stored in a list of ByteBuffers. Smaller values may be copied into a shared ByteBuffer. Larger
 * values are stored in dedicated ByteBuffers, possibly by creating a zero-copy ByteBuffer wrapping
 * existing storage of the value.
 */
abstract class ProtoValueMessageGroupBase extends ProtoKeyedMessageGroup {
  ...
  // Maximum number of value bytes to allow in a single message. The sum of the length of all values
  // in
  ...
  public static final int maxValueBytesPerMessage =
      DHTConstants.defaultFragmentationThreshold + 1024 * 1024;
  // Values below this size are copied into a common buffer. Values above this size are contained in
  // dedicated
  ...
  public ProtoValueMessageGroupBase(
      MessageType type,
      UUIDBase uuid,
      long context,
      int opSize,
