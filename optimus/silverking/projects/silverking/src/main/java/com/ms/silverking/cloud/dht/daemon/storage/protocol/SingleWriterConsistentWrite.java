...
public class SingleWriterConsistentWrite
    extends BaseStorageOperation<SingleWriterConsistentStorageEntryState> {
  private static final boolean debug = false;

  private static Logger log = LoggerFactory.getLogger(SingleWriterConsistentWrite.class);

  public SingleWriterConsistentWrite(
  ...
  public void update(
    ...
    assert replica != null;
    if (update == OpResult.SUCCEEDED
        || update == OpResult.INVALID_VERSION
        || update == OpResult.MUTATION
        || update == OpResult.NO_SUCH_NAMESPACE
        || update == OpResult.LOCKED) {
      SingleWriterConsistentStorageEntryState entryState;

      if (debug) {
        System.out.printf(
            "key %s replica %s\tupdate %s\n", KeyUtil.keyToString(key), replica, update);
      }
      ...
      synchronized (entryState) {
        if (entryState.getCurOpResult() == OpResult.INCOMPLETE) {
          ...
          if (debug) {
            System.out.printf(
                "key %s prevState %s nextState %s transitionResult %s\n",
  ...
  public void initializeEntryState(
    ...
    setEntryState(
        entryKey, new SingleWriterConsistentStorageEntryState(primaryReplicas, secondaryReplicas));
  }

  public byte nextStorageState(byte prevStorageState) {
    return (byte)
        TwoPhaseStorageState.nextState(TwoPhaseStorageState.values[prevStorageState]).ordinal();
  }
}
