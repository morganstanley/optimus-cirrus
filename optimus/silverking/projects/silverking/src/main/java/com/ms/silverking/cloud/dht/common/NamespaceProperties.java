...
public class NamespaceProperties {
  ...
  // For backward compatibility only (we drop "creationTime" and "name", since its not in the old
  // data)
  ...
  public NamespaceProperties(
      NamespaceOptions options, String name, String parent, long minVersion, long creationTime) {
    assert options != null;
  ...
  public NamespaceProperties(
      NamespaceOptions options, String name, String parent, long minVersion) {
  ...
  public boolean equals(Object o) {
    ...
    return minVersion == oProperties.minVersion
        && creationTime == oProperties.creationTime
        && options.equals(oProperties.options)
        && Objects.equals(parent, oProperties.parent)
        && Objects.equals(name, oProperties.name);
  }
  ...
  public boolean partialEquals(NamespaceProperties other) {
    ...
    // 2. Conditionally check name, new version silverking ensures client will always enrich
    // nsProperties with name;
  ...
  public static NamespaceProperties parse(String def, long creationTime) {
    ...
    if (nsProperties.hasCreationTime()) {
      log.info(
          "The parsed NamespaceProperties already has creationTime ["
              + nsProperties.getCreationTime()
              + "] when "
              + "trying to parse it with creationTime ["
              + creationTime
              + "]");
    }
