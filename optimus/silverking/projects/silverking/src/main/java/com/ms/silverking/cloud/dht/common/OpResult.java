...
/**
 * Internal representation of the result of an operation passed back by methods and stored for
 * ongoing operations.
 *
 * <p>The client API uses OperationState and FailureCause to obtain similar information.
 */
public enum OpResult {
  ...
  // success
  SUCCEEDED,
  SUCCEEDED_AND_NO_SUCH_VALUE, // DEPRECATED - left for compatibility. Do not reuse this position in
  // the enum
  // failure
  ERROR,
  TIMEOUT,
  ...
  NO_SUCH_VALUE,
  SIMULTANEOUS_PUT,
  MULTIPLE, // Multiple means multiple OpResults for keys in a batch (e.g. 1 Succeed, 1 Error). This
  // is seen as a Failure for the whole batch
  INVALID_VERSION,
  NO_SUCH_NAMESPACE,
  CORRUPT,
  LOCKED,
  ...
  public boolean hasFailed() {
    if (this == SUCCEEDED || this == INCOMPLETE) {
      ...
      if (this != NO_SUCH_VALUE) {
        ...
        // For a context where NO_SUCH_VALUE may exist, the NonExistenceResponse version must be
        // used
        // throw new RuntimeException("Unexpected NO_SUCH_VALUE in hasFailed() ");
        // We allow this as setting the OpResult may occur after results are added, due to async
        // access
        return false;
      }
    }
      ...
      case NO_SUCH_VALUE:
        if (nonExistenceResponse == NonExistenceResponse.NULL_VALUE) {
          throw new RuntimeException(
              "toFailureCause() can't be called for "
                  + this
                  + " with NonExistenceResponse.NULL_VALUE");
        } else {
