...
/**
 * Simplifies constraint checking. Similar to assertion except that constraints are not disabled.
 * Also similar to Guava Precondition class except that this is intended for general usage, not just
...
public class Constraint {
  ...
  public static void checkBounds(int min, int max, int i, String msg) {
    ...
    if (i < min) {
      throw new ConstraintViolationException(
          "checkBounds min bound failed: " + i + " < " + min + " " + msg);
    } else if (i > max) {
      throw new ConstraintViolationException(
          "checkBounds max bound failed: " + i + " > " + max + " " + msg);
    }
  }
}
