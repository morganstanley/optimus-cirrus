...
public class SSUtil {
  ...
  public static ByteBuffer retrievalResultBufferFromValue(
      ByteBuffer value, SSStorageParameters storageParams, SSRetrievalOptions options) {
    boolean returnValue = options.getRetrievalType().hasValue();
    ByteBuffer buf =
        ByteBuffer.allocate(
            MetaDataUtil.computeStoredLength(
                returnValue ? storageParams.getCompressedSize() : 0,
                storageParams.getChecksumType().length(),
                emptyUserData.length));
    StorageFormat.writeToBuf(
        null,
        value,
  ...
  public static byte[] metaDataToStoredValue(SSStorageParameters storageParams) {
    ByteBuffer buf =
        ByteBuffer.allocate(
            MetaDataUtil.computeMetaDataLength(
