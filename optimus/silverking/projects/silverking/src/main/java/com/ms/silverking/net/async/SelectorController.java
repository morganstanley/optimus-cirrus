...
/**
 * Contains a single thread that monitors an NIO Selector passing events to the appropriate worker.
 *
 * <p>Note that multiple SelectorControllers may share output workers. This allows multiple
 * selection threads to be used to pump events to common event processing workers.
 *
 * <p>Any given Connection is only registered with a single SelectorController.
 */
public class SelectorController<C extends Connection> implements Runnable {
  ...
  private final BaseWorker<Triple<ServerSocketChannel, SelectorController, SelectionKey>>
      acceptWorker;
  ...
  private static ConcurrentMap<String, AtomicInteger> ccSelectorThreadIDs =
      new ConcurrentHashMap<>();

  public SelectorController(
      BaseWorker<Triple<ServerSocketChannel, SelectorController, SelectionKey>> acceptWorker,
      BaseWorker<C> connectWorker,
      BaseWorker<C> readWorker,
      BaseWorker<C> writeWorker,
  ...
  private void doSelection() throws IOException {
    ...
    // Right here somebody could have wanted a wakeup, but this will be ignored because pending was
    // true.
  ...
  public void addServerChannel(
      ServerSocketChannel serverChannel, ChannelRegistrationWorker crWorker) {
    if (AsyncGlobals.debug && debug) {
  ...
  private void processKeyChangeRequest(KeyChangeRequest keyChangeRequest) {
    ...
    switch (keyChangeRequest.getType()) {
      ...
      case ADD_OPS:
        ...
        if (key == null) {
          log.info(
              "Unexpected null keyChangeRequest.getChannel().keyFor(selector): "
                  + keyChangeRequest
                  + " "
                  + selector);
        } else {
