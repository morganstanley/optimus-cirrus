...
public class WritableCuckooConfig extends CuckooConfig {
  ...
  public WritableCuckooConfig(CuckooConfig cuckooConfig, int cuckooLimit) {
    this(cuckooConfig.getTotalEntries(), cuckooConfig.getNumSubTables(), cuckooConfig.getEntriesPerBucket(),
        cuckooLimit);
  }

  public static WritableCuckooConfig nonWritableConfig(int totalEntries, int numSubTables, int entriesPerBucket) {
  ...
  public static WritableCuckooConfig nonWritableConfig(CuckooConfig cuckooConfig) {
    return nonWritableConfig(cuckooConfig.getTotalEntries(), cuckooConfig.getNumSubTables(),
        cuckooConfig.getEntriesPerBucket());
  }

  public WritableCuckooConfig doubleEntries() {
    return new WritableCuckooConfig(getTotalEntries() * 2, getNumSubTables(), getEntriesPerBucket(), getCuckooLimit());
  }

  public WritableCuckooConfig newTotalEntries(int totalEntries) {
    return new WritableCuckooConfig(totalEntries, getNumSubTables(), getEntriesPerBucket(), getCuckooLimit());
  }
