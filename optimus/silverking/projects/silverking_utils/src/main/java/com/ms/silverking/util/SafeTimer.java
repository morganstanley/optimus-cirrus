...
/**
 * Extends Timer to ensure that all called TimerTasks are wrapped in SafeTimerTasks. This prevents
 * the Timer from breaking due to a thrown exception in a TimerTask.
 */
public class SafeTimer extends Timer {
   ...
   * @throws IllegalArgumentException if <tt>time.getTime()</tt> is negative, or the timer task is
   *     not a SafeTimerTask
   * @throws IllegalStateException if task was already scheduled or cancelled, timer was cancelled
   ...
   *     System.currentTimeMillis() < 0}, or {@code period <= 0} ,or the timer task is not a
   *     SafeTimerTask
   * @throws IllegalStateException if task was already scheduled or cancelled, timer was cancelled,
   *     or timer thread terminated.
   * @throws NullPointerException if {@code task} is null
   ...
   * long run, the frequency of execution will generally be slightly lower than the reciprocal of
   * the specified period (assuming the system clock underlying <tt>Object.wait(long)</tt> is
   * accurate). As a consequence of the above, if the scheduled first time is in the past, it is
   ...
   * maintenance every day at a particular time. It is also appropriate for recurring activities
   ...
   * to "catch up." In the long run, the frequency of execution will be exactly the reciprocal of
   * the specified period (assuming the system clock underlying <tt>Object.wait(long)</tt> is
   * accurate). As a consequence of the above, if the scheduled first time is in the past, then any
   * "missed" executions will be scheduled for immediate "catch up" execution.
   *
   * <p>Fixed-rate execution is appropriate for recurring activities that are sensitive to
   * <i>absolute</i> time, such as ringing a chime every hour on the hour, or running scheduled
   * maintenance every day at a particular time. It is also appropriate for recurring activities
