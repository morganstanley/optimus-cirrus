...
trait UpsertableTransactionInfo {
  ...
  def businessEvent: SerializedBusinessEvent = putEvent.bes.singleOrNone
  ...
  def serializedEntityClassNames: Set[String] = allSerializedEntities.map(_.className).toSet
