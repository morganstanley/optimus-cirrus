...
object ImmutableArray {
  ...
  private def canBuildFrom[T: ClassTag, Node[_]]: CanBuildFrom[ImmutableArray[Node[T]], T, ImmutableArray[T]] = {
    new CanBuildFrom[ImmutableArray[Node[T]], T, ImmutableArray[T]] {
      ...
      override def newBuilder(from: ImmutableArray[Node[T]]): mutable.Builder[T, ImmutableArray[T]] = {
  ...
  implicit def companionToFactory[T: ClassTag](
      factory: ImmutableArray.type
  ): Factory[T, ImmutableArray[T]] = new ImmutableArrayFactory[T]

  class ImmutableArrayFactory[A: ClassTag] extends Factory[A, ImmutableArray[A]] {
    override def fromSpecific(it: IterableOnce[A]): ImmutableArray[A] = ImmutableArray.wrapped(Array.from(it))
    override def newBuilder: mutable.Builder[A, ImmutableArray[A]] = new mutable.Builder[A, ImmutableArray[A]] {
