...
trait CovariantSet[+A]
  ...
  override def concat[B >: A](xs: IterableOnce[B]): CovariantSet[B] =
    xs.iterator.foldLeft(this.asCovariantSet[B])(_ + _)

  /** Creates a new set from this set with some elements removed */
  def -[B >: A](elem1: B, elem2: B, elems: B*): CovariantSet[A] = this - elem1 - elem2 -- elems

  /** Creates a new set from this set by removing all elements of another collection */
  def --[B >: A](xs: IterableOnce[B]): CovariantSet[A] = xs.iterator.foldLeft(coll)(_ - _)

  /** Tests whether this set is a subset of another set */
  def subsetOf[B >: A](that: CovariantSet[B]): Boolean = this forall that

  /** Computes the intersection between this set and another set */
  def intersect[B >: A](that: CovariantSet[B]): CovariantSet[A] = this filter that

  /** Computes the intersection between this set and another set */
  def &[B >: A](that: CovariantSet[B]): CovariantSet[A] = this intersect that
  ...
  def diff[B >: A](that: CovariantSet[B]): CovariantSet[A] = this -- that

  /** Computes the difference between this set and another set */
  def &~[B >: A](that: CovariantSet[B]): CovariantSet[A] = this diff that
  ...
  override def toSeq: collection.immutable.Seq[A] = toList
  ...
  override def hashCode() = scala.util.hashing.MurmurHash3.unorderedHash(coll, setSeed)
  override def iterableFactory: IterableFactory[CovariantSet] = CovariantSet
  override protected[this] def stringPrefix: String = "CovariantSet"
}

object CovariantSet extends IterableFactory[CovariantSet] {

  override def from[A](source: IterableOnce[A]): CovariantSet[A] = source match {
    case cs: CovariantSet[A] => cs
    case _                   => newBuilder.addAll(source).result()
  }
  override def empty[A]: CovariantSet[A] = EmptySet
  override def newBuilder[A]: MBuilder[A, CovariantSet[A]] = newBuilder(empty)
  private[platform] def newBuilder[A](empty: CovariantSet[A]): MBuilder[A, CovariantSet[A]] =
    new CovariantSetBuilder[A](empty)

  private class CovariantSetBuilder[A](empty: CovariantSet[A]) extends MBuilder[A, CovariantSet[A]] {
    protected var elems: CovariantSet[A] = empty
    def addOne(elem: A): this.type = {
      elems = (elems + elem)
      this
    }
    def clear(): Unit = { elems = empty }
    def result(): CovariantSet[A] = elems
  }
  ...
  private abstract class AbstractSet[A] extends CovariantSet[A] with Serializable {
