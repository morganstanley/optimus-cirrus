...
object Min {
  trait IntMin extends Min[Int] {
    ...
    def combine(s1: Seed, s2: Seed, state: LoopState): Seed = {
      if (s1._2(0) && s2._2(0))
        s1._1(0) = Math.min(s1._1(0), s2._1(0))
      else if (s2._2(0))
        s1._1(0) = s2._1(0)
      s1._2(0) |= s2._2(0)
      s1
  ...
  implicit object Int extends IntMin

  trait LongMin extends Min[Long] {
  ...
  implicit object Long extends LongMin

  trait DoubleMin extends Min[Double] {
  ...
  implicit object Double extends DoubleMin

  trait FloatMin extends Min[Float] {
  ...
  implicit object Float extends FloatMin

  class OrderingMin[T: TypeInfo](val ord: Ordering[T]) extends Min[T] {
  ...
  implicit def Ordering[T: TypeInfo](implicit ord: Ordering[T]): Min[T] = {
    new OrderingMin(ord)
  }
}
