...
object Registry {
  ...
  val duringCompilation = Option(System.getProperty("optimus.inCompiler")).exists(_.toBoolean)
  ...
  final def picklerOf[T: TypeTag]: Pickler[T] = runtimeOnly(picklerRegistry.lazyInstanceFor[T].eval)

  final def picklerOfType(tpe: Type): Pickler[_] = runtimeOnly(picklerRegistry.lazyInstanceFor(tpe).eval)
  ...
  final def unpicklerOf[T: TypeTag]: Unpickler[T] = runtimeOnly(unpicklerRegistry.lazyInstanceFor[T].eval)

  final def unpicklerOfType(tpe: Type): Unpickler[_] = runtimeOnly(unpicklerRegistry.lazyInstanceFor(tpe).eval)
  ...
  private def requireClassWithNoTypeParams[T](clazz: Class[T], methodName: String, alternativeMethod: String) = {
  ...
  def pickleableClasses: Set[String] = {
    picklerRegistry.pickleableClasses
  }
}
