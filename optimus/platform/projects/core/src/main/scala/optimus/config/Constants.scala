/*
 * Morgan Stanley makes this available to you under the Apache License, Version 2.0 (the "License").
 * You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0.
 * See the NOTICE file distributed with this work for additional information regarding copyright ownership.
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package optimus.config

object Constants {
  val emptyOptconf = """{ "Version": 1.0, "Cache": {} }"""

  val CONFIG_FILE_CHARSET = "UTF-8"

  // TOP LEVEL ELEMENT NAMES
  val VERSION = "Version"
  val CACHE = "Cache"
  val PGO_GEN = "PGO"

  // current/only supported version
  val CURRENTVERSION = 1.0

  // SECTION NAMES
  // TODO (OPTIMUS-15654): improve syntax of type safe config to support
  //                                                        global value section
  // private val SEC_VALUES = "Values"
  val SEC_CACHES = "Caches"
  val SEC_CONFIGS = "Configs"
  val SEC_CLASSES = "Classes"
  val SEC_CONSTRUCTORS = "Constructors"
  val SEC_ALL = SEC_CACHES :: SEC_CONFIGS :: SEC_CLASSES :: SEC_CONSTRUCTORS :: Nil

  val LegalPropertyConfigFields =
    Set("cache", "gcnative", "syncId", "localSlot", "trackForInvalidation", "twkID", "depOn", "cachePolicy")
  val LegalNodeConfigFields = Set("config", "tweakConfig", "cache")
  val LegalOutermostFields = Set("Version", "Cache", "PGO")
  val LegalEmbeddableCtorConfigFields = Set("cachePolicy", "cache")
  val LegalCacheConfigField: Set[String] = Set("name", "cacheSize", "evictOnLowMemory")
}
