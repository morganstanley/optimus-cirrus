 ...
 * </p><p>
 * The binary format for LocalDate is the Modified Julian Day Number, stored in 
 * the last 24 bits (3 bytes) of an int. This gives a date range of 1858/11/17 
 * to 47793/05/02.
 * </p><p>
 * The binary format for LocalDateTime stores the date part in the first 24 bits 
 * (3 bytes) and the time offset in the day in the last 40 bits of a long. The time
 * part is stored in microsecond (1E-06) precision. This ordering of date and time 
 * allows for direct comparison and sorting of the binary values. Microsecond precision
 * needs 37 bits, so 3 bits are unused.
 *  </p>
 *
 */
public class DateTimeStorable {
	...
	public static int storeDate( LocalDate date ) throws IllegalArgumentException {
		...
		if( mjd > maxMJD || mjd < 0 ) {
			throw new IllegalArgumentException( String.format(
					"Cannot store %d as julian days, out of range.", mjd ) );
		}
		return (int) mjd;
	}
	...
	public static LocalDate restoreDate( int binary ) throws IllegalArgumentException  {
		...
		if( mjd > maxMJD || mjd < 0 ) {
			throw new IllegalArgumentException( String.format(
					"Cannot restore %d as julian days, out of range.", mjd ) );
		}
	...
	public static LocalDateTime restoreDateTime( long binary ) throws IllegalArgumentException {
		...
		if( micros > maxMicros || micros < 0 ) {
			throw new IllegalArgumentException( String.format(
					"Cannot restore %d micros, out of range. Max = %d", micros, maxMicros ) );
		}
