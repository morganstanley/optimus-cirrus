...
public final class ZonedDateTimeOps {
  ...
  // JSR310 accepted short ids for timezones (see java.time.ZoneId.SHORT_IDS), but java.time does not.
  //
  // Additionally in Java 8 there is a difference in parsing behaviour with datetime strings that contain
  ...
  //
  // scala> javax.time.calendar.ZonedDateTime.parse("2016-11-10T23:59:59.210-04:00[US/Eastern]").toInstant
  ...
  // but the short id issue remains, and because ZonedDateTime.parse is being used in places where the strings come
  ...
  public static ZonedDateTime parseTreatingOffsetAndZoneIdLikeJSR310(CharSequence chars) {
    try {
      ...
      // See if it failed because of use of short ids in the string. To be compatible with javax.time
      ...
      if (fixed != null) {
        ...
        // Not the format we aim to deal with here so throw original exception. JSR310 would have done the same.
        throw e;
      }
    }
  }
